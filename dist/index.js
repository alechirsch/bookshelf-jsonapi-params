...
var _jsonFields = require('./json-fields');

var _jsonFields2 = _interopRequireDefault(_jsonFields);
...
exports.default = function (Bookshelf) {
    ...
    var fetchJsonApi = function fetchJsonApi(opts) {
        ...
        internals.buildDependenciesHelper = function (column, relationHash) {
            ...
            var _column$split = column.split(':'),
                _column$split2 = _slicedToArray(_column$split, 1),
                key = _column$split2[0];
                ...
                (0, _lodash.forEach)(fieldNames, function (fieldValue, fieldKey) {
                    ...
                    fieldNames[fieldKey] = (0, _lodash.map)(fieldNames[fieldKey], function (value) {
                        ...
                        var _value$split = value.split(':'),
                            ...
                            column = _value$split2[0],
                            jsonColumn = _value$split2[1],
                            dataType = _value$split2[2];
                        ...
                        internals.model.query(function (qb) {
                            ...
                            (0, _lodash.forEach)(fieldNames[fieldKey], function (column) {
                                ...
                                if (column.aggregateFunction) {
                                    ...
                                    var _column$split3 = column.split(':'),
                                        _column$split4 = _slicedToArray(_column$split3, 3),
                                        columnToSelect = _column$split4[0],
                                        jsonColumn = _column$split4[1],
                                        dataType = _column$split4[2];
                        ...
                        if ((0, _lodash.isObjectLike)(value)) {
                            ...
                            if ((0, _lodash.includes)(filterTypes, key)) {
                                ...
                                (0, _lodash.forEach)(filterTypeValues, function (typeValue, typeKey) {
                                    var _typeKey$split = typeKey.split(':'),
                                        _typeKey$split2 = _slicedToArray(_typeKey$split, 3),
                                        column = _typeKey$split2[0],
                                        jsonColumn = _typeKey$split2[1],
                                        dataType = _typeKey$split2[2];
                                    ...
                                    var valueArray = (0, _splitString2.default)(String(typeValue), { keepQuotes: true, sep: ',' });
                                    ...
                                    if (jsonColumn) {
                                        ...
                                        _jsonFields2.default.buildFilterWithType(qb, Bookshelf.knex, key, valueArray, column, jsonColumn, dataType, extraEqualityFilter);
                                    } else {
                                        ...
                                        if (key === 'like') {
                                            qb.where(function (qbWhere) {
                                                ...
                                                (0, _lodash.forEach)(valueArray, function (val) {
                                                    ...
                                                    qbWhere[where](Bookshelf.knex.raw('LOWER(CAST(:column: AS ' + internals.textType + ')) like LOWER(:value)', {
                                                        value: '%' + val + '%',
                                                        column: column
                                                    }));
                                            ...
                                            var hasNull = valueArray.length !== (0, _lodash.pull)(valueArray, null, 'null').length;
                                            if (hasNull) {
                        ...
                        // If the value is an equality filter
                                ...
                                if (!(0, _lodash.hasIn)(filterValues.like, key)) {
                                    var _key$split = key.split(':'),
                                        ...
                                        jsonColumn = _key$split2[1],
                                        dataType = _key$split2[2];
                                    // Remove all but the last table name, need to get number of dots
                                    ...
                                    if (jsonColumn) {
                                        _jsonFields2.default.buildFilterWithType(qb, Bookshelf.knex, 'equal', value, column, jsonColumn, dataType);
                                    } else {
         ...
         * @param column
         */
        internals.equalityFilter = function (qb, column, value) {
            var whereType = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'where';


            var hasNull = value.length !== (0, _lodash.pull)(value, null, 'null').length;
            if (hasNull) {
        ...
        internals.formatRelation = function (attribute) {
            var _attribute$split = attribute.split(':'),
                _attribute$split2 = _slicedToArray(_attribute$split, 3),
                column = _attribute$split2[0],
                jsonColumn = _attribute$split2[1],
                dataType = _attribute$split2[2];
        ...
        internals.buildSort = function () {
            ...
            if ((0, _lodash.isArray)(sortValues) && !(0, _lodash.isEmpty)(sortValues)) {
                ...
                (0, _lodash.forEach)(sortValues, function (sortBy) {
                    ...
                    var sortType = sortDesc.indexOf(sortBy) === -1 ? 'asc' : 'desc';
                    if (sortBy) {
                        var _sortBy$split = sortBy.split(':'),
                            _sortBy$split2 = _slicedToArray(_sortBy$split, 3),
                            column = _sortBy$split2[0],
                            jsonColumn = _sortBy$split2[1],
                            dataType = _sortBy$split2[2];

                        column = internals.formatRelation(column);
                        if (jsonColumn) {
                            internals.model.query(function (qb) {

                                _jsonFields2.default.buildSort(qb, sortType, column, jsonColumn, dataType);
                            });
                        } else {
        ...
        internals.formatColumnCollection = function () {
            var columnNames = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];


            return (0, _lodash.map)(columnNames, function (columnName) {
                var _columnName$split = columnName.split(':'),
                    _columnName$split2 = _slicedToArray(_columnName$split, 3),
                    column = _columnName$split2[0],
                    jsonColumn = _columnName$split2[1],
                    dataType = _columnName$split2[2];

                var columnComponents = column.split('.');
                var lastIndex = columnComponents.length - 1;
                var tableAttribute = columnComponents[lastIndex];
                var formattedTableAttribute = (0, _lodash.keys)(_this.format(_defineProperty({}, tableAttribute, undefined)))[0];
                columnComponents[lastIndex] = formattedTableAttribute;

                return (0, _lodash.filter)([columnComponents.join('.'), jsonColumn, dataType]).join(':');
            });
        };
        ...
        internals.formatColumnNames = function () {
            ...
            (0, _lodash.forOwn)(columnNames, function (columnCollection, columnNameKey) {
